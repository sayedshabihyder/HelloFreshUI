...
public class SummaryReport implements IReporter {
	...
	private static final Logger LOG = Logger.getLogger(SummaryReport.class);
	...
	private static String outFilename = "SummaryReport.html";
	...
	protected void writeSuiteSummary() {
		...
		writer.print("<th> Durations(Seconds)</th>");
		...
		for (SuiteResult suiteResult : suiteResults) {
			for (TestResult testResult : suiteResult.getTestResults()) {
				...
				int total = passedTests + skippedTests + failedTests;
				...
				buffer.setLength(0);
				writeTableData(buffer.append("<a>").append(Utils.escapeHtml(testResult.getTestName())).append("</a>")
						.toString());
				...
				writeTableData(integerFormat.format(total), "num");
	...
	protected void writeScenarioSummary() {
		...
		for (SuiteResult suiteResult : suiteResults) {
			...
			for (TestResult testResult : suiteResult.getTestResults()) {
				...
				testName = Utils.escapeHtml(testResult.getTestName());
		...
		if (!classResults.isEmpty()) {
			...
			for (ClassResult classResult : classResults) {
				...
				for (MethodResult methodResult : classResult.getMethodResults()) {
					...
					for (ITestResult result : results) {
						...
						for (Object parameter : result.getParameters()) {
							...
							str = str + Utils.escapeHtml(Utils.toString(parameter, Object.class)) + "\n";
				...
				writer.print(Utils.escapeHtml(
						classResult.getClassName().substring(classResult.getClassName().lastIndexOf(".") + 1)));
				writer.print("</td>");
				writer.print(buffer);
	 ...
	 *            the HTML contents
	 * @param cssClasses
	 *            the space-delimited CSS classes or null if there are no classes to
	 *            apply
	 */
